...
"""Adaptive solution of a Poisson problem on an L-shaped domain
...
# --- Mesh generation
def create_lshape(h: float) -> dmesh.Mesh:
    # Initialise gmsh
    gmsh.initialize()
    ...
    domain_mesh, _, _ = gmshio.model_to_mesh(gmsh.model, MPI.COMM_WORLD, 0, gdim=2)
...
# --- The primal problem
def solve(domain: AdaptiveLShape, degree: int) -> dfem.Function:
    ...
    outfile = dolfinx.io.XDMFFile(MPI.COMM_WORLD, "solution.xdmf", "w")
...
def estimate(
    ...
    form_eta = dfem.form(ufl.inner(delta_sigmaR, delta_sigmaR) * v * ufl.dx)
...
def post_process(
    ...
    err_h1 = np.sqrt(
        dfem.assemble_scalar(
            dfem.form(ufl.inner(ufl.grad(err_W), ufl.grad(err_W)) * ufl.dx)
        )
    )
...
def adaptive_solver(order_prime: int, order_eqlb: int, nref: int, doerfler: int):
    ...
    # Storage of results
    outname_base = "LShape_P{}_RT{}".format(order_prime, order_eqlb)
    ...
    for n in range(0, nref):
        ...
        # Equilibrate the flux
        delta_sigmaR = equilibrate(domain, -ufl.grad(uh), order_eqlb, True)
