...
def solve(
    ...
    solver = PETSc.KSP().create(domain.mesh.comm)
...
def time_problem(
    ...
    # Storage results
    if guarantied_ee:
        outname_base = "Timing_UnifCook-P{}_RT{}_gEE".format(order_prime, order_eqlb)
    else:
        outname_base = "Timing_UnifCook-P{}_RT{}_hEI".format(order_prime, order_eqlb)
    ...
    for n in range(nretry):
        ...
        timings_n = adaptive_solver(
            order_prime, order_eqlb, h0, nref, 1.0, guarantied_ee, solver_type
        )
...
if __name__ == "__main__":
    ...
    time_problem(2, 2, 10, 7, True, SolverType.mumps, nretry)
    time_problem(2, 2, 10, 7, False, SolverType.mumps, nretry)

    # Solve based on P2 with equilibration in RT3
    time_problem(2, 3, 10, 7, True, SolverType.mumps, nretry)
    time_problem(2, 3, 10, 7, False, SolverType.mumps, nretry)

    # Solve based on P3 with equilibration in RT3
    time_problem(3, 3, 16.3, 7, True, SolverType.mumps, nretry)
    time_problem(3, 3, 16.3, 7, False, SolverType.mumps, nretry)

    # Solve based on P3 with equilibration in RT4
    time_problem(3, 4, 16.3, 7, True, SolverType.mumps, nretry)
    time_problem(3, 4, 16.3, 7, False, SolverType.mumps, nretry)
